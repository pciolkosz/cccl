...
private:
  ...
  struct opstate_t : cudax::_immovable
  {
    ...
    using completion_signatures   = cudax::completion_signatures<cudax::set_value_t(), cudax::set_stopped_t()>;
    ...
    void start() & noexcept
    {
      ...
      data_->all_commands_.emplace_back([this]() {
        if (cudax::get_stop_token(cudax::get_env(rcvr_)).stop_requested())
        {
          cudax::set_stopped(static_cast<Rcvr&&>(rcvr_));
        }
        else
        {
          cudax::set_value(static_cast<Rcvr&&>(rcvr_));
        }
      });
  ...
  struct env_t
  {
    ...
    impulse_scheduler query(cudax::get_completion_scheduler_t<cudax::set_value_t>) const noexcept
    {
      return impulse_scheduler{data_};
    }

    impulse_scheduler query(cudax::get_completion_scheduler_t<cudax::set_stopped_t>) const noexcept
    {
      return impulse_scheduler{data_};
    }
  };
