...
#include "../test/async/common/catch2.hpp"
...
void check_values(Sndr&& sndr, const Values&... values) noexcept
{
  try
  {
    auto opt = cudax::sync_wait(static_cast<Sndr&&>(sndr));
    if (!opt)
    {
...
template <class... Ts>
using types = cudax::_mlist<Ts...>;

template <class... Values, class Sndr>
USTDEX_HOST_DEVICE void check_value_types(Sndr&& sndr) noexcept
{
  using actual_t   = cudax::value_types_of_t<Sndr, cudax::env<>, types, cudax::_mmake_set>;
  using expected_t = cudax::_mmake_set<Values...>;

  static_assert(cudax::_mset_eq<expected_t, actual_t>, "value_types_of_t does not match expected types");
}

template <class... Errors, class Sndr>
USTDEX_HOST_DEVICE void check_error_types(Sndr&& sndr) noexcept
{
  using actual_t   = cudax::error_types_of_t<Sndr, cudax::env<>, cudax::_mmake_set>;
  using expected_t = cudax::_mmake_set<Errors...>;

  static_assert(cudax::_mset_eq<expected_t, actual_t>, "error_types_of_t does not match expected types");
}

template <bool SendsStopped, class Sndr>
USTDEX_HOST_DEVICE void check_sends_stopped(Sndr&& sndr) noexcept
{
  static_assert(cudax::sends_stopped<Sndr> == SendsStopped, "sends_stopped does not match expected value");
}
