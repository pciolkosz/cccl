...
#  include <cuda/__memory_resource/overalign_allocation.h>
...
inline constexpr bool __buffer_compatible_env =
  ...
  || ::cuda::std::execution::__queryable_with<const _Env&, overalign_allocation_t>;
...
private:
  ...
  template <class _Env>
  static constexpr size_t __alignment_from_env(const _Env& __env) noexcept
  {
    if constexpr (::cuda::std::execution::__queryable_with<const _Env&, overalign_allocation_t>)
    {
      return ::cuda::overalign_allocation(__env);
    }
    else
    {
      return alignof(_Tp);
...
#  endif // _CCCL_DOXYGEN_INVOKED
  ...
  [[nodiscard]] _CCCL_HIDE_FROM_ABI friend constexpr ::cuda::std::size_t
  get_property(const buffer& __self, overalign_allocation_t) noexcept
  {
    return __self.__buf_.alignment();
  }
};
