  ...
  ::cuda::std::uint32_t __txCount)
{
  ...
  // __space == space_shared (due to parameter type constraint)
    if constexpr (__scope == scope_cta) {
      ...
      asm (
        "mbarrier.complete_tx.relaxed.cluster.shared::cta.b64 [%0], %1;"
        :
        : "r"(__as_ptr_smem(__addr)),
          "r"(__txCount)
        : "memory"
  ...
  ::cuda::std::uint32_t __txCount)
{
  ...
  // __space == space_cluster (due to parameter type constraint)
    if constexpr (__scope == scope_cta) {
      asm (
        "mbarrier.complete_tx.relaxed.cta.shared::cluster.b64 [%0], %1;"
        :
  ...
  ::cuda::std::uint32_t __ctaMask)
{
  ...
  // __space == space_cluster (due to parameter type constraint)
    if constexpr (__scope == scope_cta) {
      asm (
        "mbarrier.complete_tx.relaxed.cta.shared::cluster.multicast::cluster::32b.b64 [%0], %1, %2;"
        :
        : "r"(__as_ptr_dsmem(__addr)),
          "r"(__txCount),
          "r"(__ctaMask)
      ...
      asm (
        "mbarrier.complete_tx.relaxed.cluster.shared::cluster.multicast::cluster::32b.b64 [%0], %1, %2;"
        :
        : "r"(__as_ptr_dsmem(__addr)),
          "r"(__txCount),
          "r"(__ctaMask)
