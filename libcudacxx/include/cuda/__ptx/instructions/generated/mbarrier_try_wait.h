...
/*
// mbarrier.try_wait.phase_type.sem.scope.shared::cta.b64 waitComplete|isReportSeen, reportValue, [addr], state; // PTX
ISA 94, SM_90
// .phase_type = { .phase_type::primary }
// .sem       = { .acquire, .relaxed }
// .scope     = { .cta, .cluster }
...
_CCCL_DEVICE static inline bool mbarrier_try_wait(
  ...
  ::cuda::ptx::scope_t<_Scope> __scope,
  bool& __isReportSeen,
  ::cuda::std::uint32_t& __reportValue,
  ::cuda::std::uint64_t* __addr,
  ::cuda::std::uint64_t __state)
{
  // __phase_type == mbarrier_phase_primary (due to parameter type constraint)
  ...
  if constexpr (__sem == sem_acquire && __scope == scope_cta)
  {
    asm("{\n\t"
        ".reg .pred P_OUT_waitComplete; \n\t"
        ".reg .pred P_OUT_isReportSeen; \n\t"
        "mbarrier.try_wait.phase_type::primary.acquire.cta.shared::cta.b64 P_OUT_waitComplete|P_OUT_isReportSeen, %2, "
        "[%3], %4; \n\t"
        "selp.b32 %0, 1, 0, P_OUT_waitComplete; \n\t"
        "selp.b32 %1, 1, 0, P_OUT_isReportSeen; \n"
        "}"
        : "=r"(__waitComplete), "=r"(__isReportSeen_tmp), "=r"(__reportValue)
        : "r"(__as_ptr_smem(__addr)), "l"(__state)
        : "memory");
  }
  else if constexpr (__sem == sem_acquire && __scope == scope_cluster)
  ...
  else if constexpr (__sem == sem_relaxed && __scope == scope_cta)
  {
    asm("{\n\t"
        ".reg .pred P_OUT_waitComplete; \n\t"
        ".reg .pred P_OUT_isReportSeen; \n\t"
        "mbarrier.try_wait.phase_type::primary.relaxed.cta.shared::cta.b64 P_OUT_waitComplete|P_OUT_isReportSeen, %2, "
        "[%3], %4; \n\t"
        "selp.b32 %0, 1, 0, P_OUT_waitComplete; \n\t"
        "selp.b32 %1, 1, 0, P_OUT_isReportSeen; \n"
        "}"
...
/*
// mbarrier.try_wait.phase_type.sem.scope.shared::cta.b64 waitComplete|isReportSeen, reportValue, [addr], state,
