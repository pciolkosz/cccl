...
_CCCL_DEVICE static inline void ldmatrix_m16n16_trans(
  ::cuda::ptx::space_shared_t,
  ::cuda::std::uint32_t (&__out_var)[2],
  const _B8* __smem_ptr)
{
  ...
  const void* __smem_ptr)
{
  // __space == space_shared (due to parameter type constraint)
    asm (
      "ldmatrix.sync.aligned.m16n16.x1.trans.shared.b8x16.b6x16_p32 {%0}, [%1];"
      : "=r"(__out_var[0])
      : "r"(__as_ptr_smem(__smem_ptr))
      : "memory"
    );
  ...
  const void* __smem_ptr)
{
  // __space == space_shared (due to parameter type constraint)
    asm (
      "ldmatrix.sync.aligned.m16n16.x1.trans.shared.b8x16.b4x16_p64 {%0}, [%1];"
      : "=r"(__out_var[0])
      : "r"(__as_ptr_smem(__smem_ptr))
      : "memory"
    );
  ...
  const void* __smem_ptr)
{
  // __space == space_shared (due to parameter type constraint)
    asm (
      "ldmatrix.sync.aligned.m16n16.x2.trans.shared.b8x16.b6x16_p32 {%0, %1}, [%2];"
      : "=r"(__out_var[0]),
        "=r"(__out_var[1])
      : "r"(__as_ptr_smem(__smem_ptr))
      : "memory"
  ...
  const void* __smem_ptr)
{
  // __space == space_shared (due to parameter type constraint)
    asm (
      "ldmatrix.sync.aligned.m16n16.x2.trans.shared.b8x16.b4x16_p64 {%0, %1}, [%2];"
      : "=r"(__out_var[0]),
        "=r"(__out_var[1])
      : "r"(__as_ptr_smem(__smem_ptr))
      : "memory"
